...
def plot(indices, plot_data, path, epoch, img_res, plot_nimgs):
	if plot_data is not None:
		#cam_loc, cam_dir = render_utils.get_camera_for_plot(plot_data['pose'])
		plot_images(plot_data['rgb_eval'], plot_data['rgb_gt'], path, epoch, plot_nimgs, img_res)
		data = []
		# plot_point_cloud(plot_data['point_cloud'], path, epoch)
		point_cloud_trace = get_3D_scatter_trace(plot_data['point_cloud'],
		                                         name='',
		                                         size=2,
		                                         color='magenta',
		                                         caption='point_cloud')

		samples_trace = get_3D_scatter_trace(plot_data['samples'],
		                                     size=2,
		                                     name='sdf',
		                                     color=T.sign(plot_data['sd_vals'].squeeze(-1)).cpu().numpy())
		data.append(point_cloud_trace)
		data.append(samples_trace)
		...
		filename = '{0}/points_{1}.html'.format(path, epoch)
		fig.write_html(filename)
		#data=[]
		# plot cam_loc + cam_dir + points + sampled_points
		#for i, loc, dir in zip(indices, cam_loc, cam_dir):
		#	data.append(get_3D_quiver_trace(loc.unsqueeze(0),
		#	                                dir.unsqueeze(0),
...
def get_3D_scatter_trace(points, name='', symbol='circle', size=3, caption=None, color=None):
...
def get_surface_trace(path, epoch, sdf, resolution=100, grid_boundary=[-2.0, 2.0], return_mesh=False, level=0):
	...
	z = []
	for i, pnts in enumerate(T.split(points, 100000, dim=0)):
...
def get_surface_high_res_mesh(sdf, resolution=100, grid_boundary=[-2.0, 2.0], level=0, take_components=True):
	...
	recon_pc = T.from_numpy(recon_pc).float().cuda()

	# Center and align the recon pc
	s_mean = recon_pc.mean(dim=0)
	s_cov = recon_pc - s_mean
	s_cov = T.mm(s_cov.transpose(0, 1), s_cov)
	vecs = T.view_as_real(T.linalg.eig(s_cov)[1].transpose(0, 1))[:, :, 0]
	if T.det(vecs) < 0:
		vecs = T.mm(T.tensor([[1, 0, 0], [0, 0, 1], [0, 1, 0]]).cuda().float(), vecs)
	helper = T.bmm(vecs.unsqueeze(0).repeat(recon_pc.shape[0], 1, 1),
	...
	g = []
	for i, pnts in enumerate(T.split(grid_points, 100000, dim=0)):
		g.append(T.bmm(vecs.unsqueeze(0).repeat(pnts.shape[0], 1, 1).transpose(1, 2),
		                   pnts.unsqueeze(-1)).squeeze() + s_mean)
	grid_points = T.cat(g, dim=0)
	...
	if (not (np.min(z) > level or np.max(z) < level)):
		...
		verts = T.from_numpy(verts).cuda().float()
		verts = T.bmm(vecs.unsqueeze(0).repeat(verts.shape[0], 1, 1).transpose(1, 2),
...
def get_surface_by_grid(grid_params, sdf, resolution=100, level=0, higher_res=False):
	...
	input_min = T.tensor(grid_params[0]).float()
	input_max = T.tensor(grid_params[1]).float()
...
def get_grid_uniform(resolution, grid_boundary=[-2.0, 2.0]):
	...
	grid_points = T.tensor(np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=T.float)
...
def get_grid(points, resolution, input_min=None, input_max=None, eps=0.1):
	if input_min is None or input_max is None:
		input_min = T.min(points, dim=0)[0].squeeze().numpy()
		input_max = T.max(points, dim=0)[0].squeeze().numpy()
	...
	xx, yy, zz = np.meshgrid(x, y, z)
	grid_points = T.tensor(np.vstack([xx.ravel(), yy.ravel(), zz.ravel()]).T, dtype=T.float).cuda()
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res):
	ground_true = ground_true.to(rgb_points.device)

	output_vs_gt = T.cat((rgb_points, ground_true), dim=0)
...
def plot_point_cloud(point_cloud, path, epoch):
	import open3d as o3d
	pcd = o3d.geometry.PointCloud()
	pcd.points = o3d.utility.Vector3dVector(point_cloud.cpu().numpy())
	o3d.visualization.draw_geometries([pcd])
	o3d.io.write_point_cloud('{0}/point_cloud_{1}.ply'.format(path, epoch), pcd)
